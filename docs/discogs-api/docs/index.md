...
Hereâs your place to code all things Discogs! The Discogs API lets developers build their own Discogs-powered applications for the web, desktop, and mobile devices. We hope the API will connect and empower a community of music lovers around the world!
...
Youâve sent a POST request to a list of resources to create a new one. The ID of the newly-created resource will be provided in the body of the response.
...
Youâre attempting to access a resource that first requires authentication. See Authenticating with OAuth.

     * **403 Forbidden**

Youâre not allowed to access this resource. Even if you authenticated, or already have, you simply donât have permission. Trying to modify another userâs profile, for example, will produce this error.
...
Youâre trying to use an HTTP verb that isnât supported by the resource. Trying to PUT to /artists/1, for example, will fail because Artists are read-only.

     * **422 Unprocessable Entity**

Your request was well-formed, but thereâs something semantically wrong with the body of the request. This can be due to malformed JSON, a parameter thatâs missing or the wrong type, or trying to perform an action that doesnât make any sense. Check the response body for specific information about what went wrong.
...
The Release Stats endpoint retrieves the total number of âhavesâ (in the communityâs collections) and âwantsâ (in the communityâs wantlists) for a given release.
...
The Master resource represents a set of similar Releases. Masters (also known as âmaster releasesâ) have a âmain releaseâ which is often the chronologically earliest.
        ...
        
        {
          ...
          "versions": [
            ...
            },
            {
              ...
              "format": "12\", White Label, 33 â RPM, Promo",
              "country": "UK",
              "title": "Plastic Dreams Mixes",
              "label": "R & S Records",
              "released": "1993",
...
The Label resource represents a label, company, recording studio, location, or other entity involved with Artists and Releases. Labels were recently expanded in scope to include things that arenât labels â the name is an artifact of this history.
...
Issue a search query to our database. This endpoint acceptspagination parameters
...
Search by combined âArtist Name - Release Titleâ title field.
...
If your application integrates YouTube videos, then third party cookies may be used. You can view YouTube and Googleâs cookie policy[here](https://www.google.com/policies/technologies/cookies/).
...
Returns the list of listings in a userâs inventory. AcceptsPagination parameters.  
...
If the listingâs status is not `For Sale`, `Draft`, or `Expired`, it cannot be modified â only deleted. To re-list a Sold listing, a new listing must be created.
...
The price of the item (in the sellerâs currency).
...
A freeform field that can be used for the sellerâs own reference. Information stored here will not be displayed to anyone other than the seller. This field is called âPrivate Commentsâ on the Discogs website.

location
    `string` (optional) **Example:** 10.00

A freeform field that is intended to help identify an itemâs physical storage location. Information stored here will not be displayed to anyone other than the seller. This field will be visible on the inventory management page and will be available in inventory exports via the website.
...
The Order resource allows you to manage a sellerâs Marketplace orders.
...
##### Body
        
        {
          ...
          "shipping_address": "Asdf Exampleton\n234 NE Asdf St.\nAsdf Town, Oregon, 14423\nUnited States\n\nPhone: 555-555-2733\nPaypal address: [[email protected]](/cdn-cgi/l/email-protection)",
...
Authentication as the seller is required.  
The response contains a `next_status` key â an array of valid next statuses for this order, which you can display to the user in (for example) a dropdown control. This also renders your application more resilient to any future changes in the order status logic.  
...
`Cancelled (Item Unavailable)`  
`Cancelled (Per Buyer's Request)`  
the orderâs current status

Furthermore, the new status must be present in the orderâs next_status list. For more information about order statuses, see Edit an order.
...
Adds a new message to the orderâs message log.  
...
Retrieve a userâs contributions by username. Accepts Pagination parameters.
...
The Collection resource allows you to view and manage a userâs collection.  
A collection is arranged into folders. Every user has two permanent folders already:

ID `0`, the âAllâ folder, which cannot have releases added to it, and  
ID `1`, the âUncategorizedâ folder.

Because itâs possible to own more than one copy of a release, each with its own notes, grading, and so on, each instance of a release in a folder has an instance ID.
...
Create a new folder in a userâs collection.
...
Edit a folderâs metadata. Folders `0` and `1` cannot be renamed.
...
Delete a folder from a userâs collection. A folder must be empty before it can be deleted.
...
View the userâs collection folders which contain a specified release. This will also show information about each release instance.
...
Returns the list of item in a folder in a userâs collection. Accepts Pagination parameters.
...
Add a release to a folder in a userâs collection.

The `folder_id` must be non-zero â you can use `1` for âUncategorizedâ.
...
Remove an instance of a release from a userâs collection folder.

To move the release to the âUncategorizedâ folder instead, use the `POST` method.
...
The new value of the field. If the fieldâs type is `dropdown`, the `value` must match one of the values in the fieldâs list of options.
...
Returns the minimum, median, and maximum value of a userâs collection.
